---
title: "halfspace_alexpiehler_sol"
author: "Alex Piehler"
date: "5 12 2019"
...
Im Nachfolgenden sind sowohl die Funktionen zu finden als auch die von Ihnen vorgegebenen Tests, sowie eine weitere Anwendung auf den `datasets::faithful`-Datensatz.

Leider ist die Implementierung nicht 100% top-down, da die `plot_depth()`-Funktion die `evaluate_depth()`-Funktion aufruft.

## Code

```{r halfspacemass functions}
train_depth <- function(data,
                        ...
                        scope = 1,
                        # the sampling region for the splitpoint within the convex set
                        seed = 111191) {
  ### Required Packages ###
  surpressMessages(require(tidyverse))
  surpressMessages(require(checkmate))
  ...
  unit_vector_list <- purrr::map(rep(list(ncol(data)), times = n_halfspace),
                                 get_unit_vector)
  ...
  mass_left <- get_mass_left(cutoffs = cutoff_points,
...
evaluate_depth <- function(data,
  ...
  checkmate::assert_subset(names(halfspaces),
                           c("hyperplanes",
                             "cutoff_points",
                             "mass_left",
                             "mass_right"))
  ...
  projections_per_datum <- make_projections_per_datum(data = data,
...
plot_depth <- function(data,
  ...
  halfspace_mass_grid <-
    evaluate_depth(data = grid_for_plot,
...
depth_fig3 <- train_depth(data_fig3, n_halfspace = 1e4, subsample = 1,
                       scope = 1, seed = 4163)
# need scope > 1 for reliable halfspace _depth_ approximation:
depth_fig3_scope15 <- train_depth(data_fig3,
                               n_halfspace = 1e3, scope = 1.5, subsample = 1,
                               seed = 4163)
gridExtra::grid.arrange(
  plot_depth(halfspaces = depth_fig3_scope15,
             data = data_fig3,
             type = "halfspacedepth",
             prespecified_grid = grid_fig3) +
    ggtitle("Tukey Halfspace Depth"),
  plot_depth(halfspaces = depth_fig3,
             data = data_fig3,
             type = "halfspacemass",
             prespecified_grid = grid_fig3) +
...
fig5 <- plot_depth(halfspaces = hs_fig5,
                   data = data_fig5[, 1:2],
                   type = "halfspacemass")
...
```{r fig 6, fig.width = 8, fig.height = 4,}
fig6 <- plot_depth(halfspaces = hs_fig5,
                   data = data_fig5[, 1:2],
                   type = "halfspacedepth")
...
```{r fig 7 old faithful mass, fig.width = 8, fig.height = 4,}
faithful <- datasets::faithful

hs_fig7 <- train_depth(faithful,
  n_halfspace = 1e4, subsample = .5
)

plot_depth(halfspaces = hs_fig7,
                   data = faithful,
                   type = "halfspacemass")
```

```{r fig 8 faithful depth, fig.width = 8, fig.height = 4,}

hs_fig8 <- train_depth(faithful,
  n_halfspace = 1e4, subsample = .5
)

plot_depth(halfspaces = hs_fig8,
                   data = faithful,
                   type = "halfspacedepth")
```